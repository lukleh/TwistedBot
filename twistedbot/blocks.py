...
class BlockDoor(Block):
	...
	def grid_bounding_box(self):
		...
		if self.is_open:
			if self.hinge_right:
				shp = self.bounding_boxes[self.facing_index - 1] if self.facing_index - 1 >= 0 else self.bounding_boxes[3]
			else:
				shp = self.bounding_boxes[self.facing_index + 1] if self.facing_index + 1 <  4 else self.bounding_boxes[0]
		else:
			shp = self.bounding_boxes[self.facing_index]
		return shp + self.coords
...
class BlockPane(BlockMultiBox):
	...
	def grid_bounding_box(self):
		out = []
		zl = self.can_connect_to(self.x, self.y, self.z - 1)
		zr = self.can_connect_to(self.x, self.y, self.z + 1)
		xl = self.can_connect_to(self.x - 1, self.y, self.z)
		xr = self.can_connect_to(self.x + 1, self.y, self.z)
