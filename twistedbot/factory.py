...
class MineCraftProtocol(Protocol):
	...
	def p_experience_orb(self, c):
		self.world.entities.new_experience_orb(eid=c.eid, count=c.count, x=c.x, y=c.y, z=c.z)
	...
	def p_encryption_key_request(self, c):
		...
		self.send_packet("encryption key response", {"shared_length": len(enc_shared_sercet), 
