...
class WalkSignsGoal(GoalBase):
	...
	def from_child(self, status):
		self.manager.grid.check_sign((self.next_waypoint[0], self.next_waypoint[1] + 1, self.next_waypoint[2]))
		if status == Status.finished or status == Status.impossible:
			self.do()
		elif status == Status.broken:
			if self.next_waypoint is not None:
				self.manager.add_subgoal(TravelToGoal, coords=self.next_waypoint)
			else:
	...
	def do(self):
		self.next_waypoint = self.next_sign(self.group)
		if self.next_waypoint is not None:
			self.manager.add_subgoal(TravelToGoal, coords=self.next_waypoint)
		else:
			self.manager_goal_return(Status.finished)
...
class GoToSignGoal(GoalBase):
	...
	def from_child(self, status):
		self.manager.grid.check_sign((self.waypoint[0], self.waypoint[1] + 1, self.waypoint[2]))
	...
	def check_state(self):
		self.waypoint = self.bot.world.navgrid.sign_waypoints.get_namepoint(self.sign_name)
	...
	def do(self):
		self.manager.add_subgoal(TravelToGoal, coords=self.waypoint)
