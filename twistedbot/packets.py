...
packets = {
	...
	52: Struct("multi block change",
		...
		UBInt16("count"),
		SBInt32("datasize"),
		MetaArray(lambda ctx: ctx.count, BitStruct("blocks",
											BitField("x", 4),
											BitField("z", 4),
											BitField("y", 8),
	...
	56: Struct("map chunk bulk",
		...
		#ByteString("data", size_name="size", encoding="zlib"),
		MetaField("data", lambda ctx: ctx["size"]),
		MetaArray(lambda ctx: ctx.count, Struct("meta",
											SBInt32("x"),
											SBInt32("z"),
...
packet_stream = Struct("packet_stream",
	OptionalGreedyRange(
		Struct("full_packet",
			UBInt8("header"),
#			IfThenElse("payload", lambda context: context["header"] != 252,
			Switch("payload", packet_stream_print_header, packets),
#				Value("data", lambda _: print_and_return(252, ""))
#			),
		),
	),
	OptionalGreedyRange(
